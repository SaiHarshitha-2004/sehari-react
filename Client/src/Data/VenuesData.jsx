...
const VenuesData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const bestVenuesResponse = await fetch("https://sehari-server.vercel.app/database/bestvenues");
        const bestVenuesData = await bestVenuesResponse.json();
        const filteredVenues = bestVenuesData.filter((ven) => isValid(ven));
        setVenues(filteredVenues);
  
        const venuesIconResponse = await fetch("https://sehari-server.vercel.app/database/venuesiconsdata");  
